 ...
 * \verbatim
 * $Author: bucknerk $ 
 * $Date: 2008/03/05 13:34:30 $ 
 * $Id: site_admin.js,v 1.4 2008/03/05 13:34:30 bucknerk Exp $
 *
 * $Log: site_admin.js,v $
...
function do_img_resize() {
  ...
  document.getElementById('i2').height=(mw*.20);
  document.getElementById('i3').height=(mw*.20);
}
...
function reg_checkscript() {
  ...
  document.forms["register"].PW2.value){
    ...
    if(document.forms["register"].USERNAME.value.length == 0 ) {
      alert("USERNAME cannot be blamk");
      return false;
      }
