 ...
 *
 * $Author: bucknerk $
 * $Date: 2008/07/18 17:36:59 $
 * $Id: data_file_functions.c,v 1.7 2008/07/18 17:36:59 bucknerk Exp $
 *
 * $Log: data_file_functions.c,v $
 * Revision 1.7  2008/07/18 17:36:59  bucknerk
 * Also fixed these to correct zone suffix errors. Check out the bottle_neck files
 ...
 *
 * Revision 1.4  2008/01/08 13:58:13  bucknerk
 * Added the global Zone_xref as the suffix that corresponds to the zone for a
 * porticular simulation.  Also added the function that sets this variable.
 *
 ...
 *
 * Revision 1.1.2.3  2007/12/03 13:30:18  bucknerk
 ...
 *
 * Revision 1.1.2.2  2007/11/28 21:15:13  bucknerk
...
#define free_abund(a,b) for(b=a; b!=NULL;b++) { free(b->data);free(b);}
#define free_flux(a,b) for(b=a; b!=NULL;b++) { free(b->data);free(b);}

/*!
 *
 * Function reads in an abundance file and gets information for the
 * isotopes of interest for each time step. 
 * 
 * The format of the abundance files is as follows:
 * abund??_cnt - the metadata file, is an integer(4 byte) step number
 * then an integer(4 byte) number-of-non-zero abundances, and last an
 * integer(4 byte) file position (this is from the beginning of the file).
 * It corresponds to the start of the record in the abund?? file from
 * the beginning of the file (SEEK_SET).
 * abund?? - the data file consists of an integer(4 byte) step number
 * followed by a set of nnz records each of which is integer(4 byte) index
 * then a double(8 byte).
 * The index should correspond to the isomap index
 *
 * Do not have to free anything because I return all the allocated memory.
 * \remark zone is not a static variable in this file
 * \param[in] n_isotopes the number of elements in the isotope array
 * \param[in] basedir the location of the saved simulation
 * \param[out] outcount the number of abundances, there should be one for
 * each timestep.
 * \return An array of Abundance structures of size outcount, each of which
 * have n_isotopes of data (which may be 0.0)
 *
 */
  

Abundance *get_abundance_data(int n_isotopes, char *basedir, 
    int *outcount)
{
  FILE *fp1,*fp2;
  int i,j,rtn,tmp;
  ...
  if(fp1) {
    ...
    perror("ERROR: Could not open last step number file:");
    return NULL;
  }
  //printf("step_num%02d: %s\n",Zone_xref,total.s);

  sprintf(file1,"%s/abund%s",basedir,Zone_xref);     // the data
  sprintf(file2,"%s/abund%s_cnt",basedir,Zone_xref); // the metadata
  
  fp1=fopen(file1,"r");
  if(fp1 == NULL) {
    perror("ERROR:Opening abundance data file:");
    fprintf(stderr,"%s\n",file1);
    return NULL;
  }
  fp2=fopen(file2,"r");
  if(fp2 == NULL) {
    perror("ERROR:Opening abundance metadata file:");
    fprintf(stderr,"%s\n",file2);
    return NULL;
  }
  // one for each time step, s+1 because they start at 0
  all_abund=(Abundance *)calloc(sizeof(Abundance),total.s+1);
  if(all_abund==NULL) {
    perror("ERROR: Allocating adbundance data storage:");
    fclose(fp1);
    fclose(fp2);
    return NULL;
  }
  ...
  if(mydata==NULL || data == NULL) {
    perror("ERROR: Allocating temporary abundance storage:");
  ...
  while(rtn) {
    for(i=0;i<rtn;i++) {
      all_abund[count].step=mydata[i].step;
//      all_abund[count].nnz=mydata[i].nnz;
      fseek(fp1,mydata[i].fpos,SEEK_SET);
      tmp=fread(&mystep,sizeof(Step),1,fp1);
      if(!tmp  || mystep.s != mydata[i].step) {
	fprintf(stderr,"ERROR=%s,%d: Error reading abundace data file",
	    __FILE__,__LINE__);
	free_abund(all_abund,bptr);
	free(mydata);
	free(data);
	fclose(fp1);
	fclose(fp2);
	return NULL;
      ...
      all_abund[count].data=(double *)calloc(sizeof(double),n_isotopes);
      if(all_abund[count].data==NULL) {
	printf("ERROR=Allocating abundance data storeage:%s\n",strerror(errno));
	free_abund(all_abund,bptr);
	free(mydata);
	free(data);
	fclose(fp1);
	...
	if(tmp) { // actually read something
	  for(j=0;j<tmp;j++) {
	    ...
	    //if(data[j].index >= isostart && data[j].index <=isoend)
            all_abund[count].data[data[j].index]=data[j].data;
	  }
	}
	else {
	  if(feof(fp1)) {
	    fprintf(stderr,"ERROR=Unexpected end of abundance data file\n");
	  }
	  else {
	    fprintf(stderr,"ERROR=Error reading abundance data file\n");
	  }
	  free_abund(all_abund,bptr);
	  free(mydata);
	  free(data);
  ...
  fclose(fp1);
  fclose(fp2);
  //printf("ending step is %d\n",all_abund[count-1].step);
  *outcount=count;
  return all_abund;
}// end of just handling the abundances.
 ...
 * then there are 4 3 bytes of internal padding between the characters and
 * the integer to align the int on the boundary.  If you call the last 9
 * bytes then there are 3 OR 7 bytes of padding depending on whether the
 ...
 * location.
 * Should only have ever call this once, because these don't change on a
 * per zone basis.
 *
 ...
 *
 * Changing this so that it takes a Z,N and only returns those isotope that
 * fall in the range
 */
...
IsoXref *get_isotope_data(char *basedir, int *outcount)
{
  ...
  while((rtn=fread((void *)&idata,1,17,fp1)) && 
      ...
      // and ensureing that the result is '\0' terminated.
      for(i=0,j=0;i<5;i++) {
  ...
  fclose(fp1);
  // Have all the data. Now I need to "weed" out those that are not 
  // wanted. First check Z if the the Z is not between Z_start and Z_end
  // inclusive throw it out. Else check Z_start's A and Z_end's A and we
  // will assume that the user wants all those between regardless of their
  // N/A value
...
int get_reaction_data(char *basedir,IsoXref *xref)
{
  ...
  //command=(char *)calloc(sizeof(char),1024);
