...
C   $Author: bucknerk $
C   $Log: net.f,v $
C   Revision 1.6  2008/07/25 16:53:30  bucknerk
C   fixing i hope
C
C   Revision 1.4  2008/07/03 20:36:48  bucknerk
C   adjusted files and tweaked comments. I think it is all compiled now. I have to test it but that can wait till Monday.
C
C   Revision 1.3  2008/07/02 18:17:08  6hx
C   Makefile updated for use with MKL 10
C   jacobian_PARDISO modified with workaround for MKL 10
C
...
!*******************************************************************************
! NOTE: This version of net.f is being used because it works with the CINA 
! code and does not effect the performance of the xnet code.  This is also
! used with the save_em_sim.f90 file.
...
!===============================================================================
      ...
      integer, parameter   :: nzmx= 1001 ! Max number of zones
      real(8), dimension(:), allocatable :: yin
      real(8) :: tdelstart,tbegin,tend,tcut,t9cut
! these next two are to read user input for start and stop times
! from a file, time_control, which has (hopefully) been created by the
! cgi handler from the time values the user input 
      real(8) :: user_start,user_stop ! added for CINA gui
...
! Save isotope and reaction mapping for use in the element synthesis visualizer
      call save_em_sim_isomap(bin_file_base)
      call save_em_sim_reacmap(bin_file_base)

! For each Zone
      Do izone=szone,nzone
        ! Write current zone to lastzone file for suite update call
        Open(21,file='lastzone')
        Write(21,'(I0)') izone
        Close(21)
...
! Open the output files for use in the element synthesis visualizer
        call save_em_sim_init(bin_file_base,ev_file_base,izone,nzone)
...
!  Find the temperature cutoff
          If(n>1) Then
	    If(tcut==0.0.and.t9h(n)<=t9cut.and.t9h(n)<t9h(n-1)) tcut=th(n)
	  Endif
        Enddo
   20   nh=n-1
        Close(8)
...
! this is the file created by the CINA gui and used to control the start
! and stop times ox XNET. This is more san than trying to modify the
! thermo profile I am copying over.  There is a problem in that multiple
! zone simulations may NOT (probably won't) want to be stopped at the
! same time. Sorry.
        Open(55,file='time_control')
        Read(55,'(e13.6/e13.6)')user_start,user_stop
        Close(55)
 
        if(user_start < tbegin) then
          write(6,'(A)') 'Selected start time is earlier than'
          write(6,'(A)') ' first thermo profile time'
          tstart=tbegin
        else
          tstart=user_start
        endif
        if(user_stop > tend) then
          write(6,'(A)') 'Selected stop time is later than'
          write(6,'(A)') ' last thermo profile time'
        else
! Don't like this but for now it should work: Kim
          tend=user_stop
        endif
...
! Close the output files for use in the element synthesis visualizer
        call save_em_sim_close()
...
!        Close(54)
      ...
      Write(6,'(A)') 'Element Synthesis Simulation Complete.'
      Stop
      End
...
!===============================================================================
      ...
      integer :: i,j,k,ig,kstep,kout,kstep_used
...
! Save data for the current time step for use in the element synthesis 
! visualizer
      call save_em_sim_tstep(kstep,kstep_used,edot)
...
! For itso>=2, output important mass fractions to the ASCII file on unit 22
      If(itso>=2) Then
        ...
        !IF ( kstep > 0) THEN
        !  Write(52,"(8es9.2)") edot
        !ENDIF
! COMMENT OUT LINES TO PREVENT  WRITING TO THE EV OUTPUT FILES
! WHICH WE ARE NOT USING IN THE cina SUITE
! LINES COMMENTED BACK IN BY EJL TO GET EDOT VALUE FOR PLOT
!        Write(52,"(i4,1es15.8,2es10.3,2es10.2,8es9.2,i4)")
!     &     kstep,t,t9t,rhot,edot,enuc,tdel,(xg(i),i=1,7),kout
!         Write(52,"(i4,1es15.8,2es10.3,2es10.2,9es12.5,i4)")
!     &    kstep,t,t9t,rhot,edot,enuc,tdel,aa(2)*y(2),aa(6)*y(6),
!     &    aa(19)*y(19),aa(20)*y(20),aa(29)*y(29),aa(30)*y(30),
!     &    aa(38)*y(38),aa(40)*y(40),kout

! For itso>=3, output time and thermo evolution to the screen
!        If(itso>=3) Write(6,"(i5,4es12.4,2i3)") 
!     &    kstep,t,tdel,t9t,rhot,knr,kts
        If(itso>=3) Write(6,"(A,I0,T16,A,1P,E16.9,T49,A,E12.5)") 
     &       'Step = ',kstep_used,'Time(sec) = ',t,' Temp(T9) = ',t9t
      Endif
      Return
      End
...
!===============================================================================
      ...
      integer :: i,kstep
      real(8), dimension(:),allocatable :: dyf
      If(.not.allocated(dyf)) Allocate(dyf(0:ny))

! Write final abundances to diagnostic output (in ASCII)
      Write(50,"(a3,2i6,2es14.7)") 'End',kstep,kstmx,tt,tstop
      Write(50,"(4(a5,es14.7,1x))") (nname(i),y(i),i=1,ny)
