...
C
C   $Author: elingerf $
C   $Id: net_setup.f,v 1.15 2009/08/07 12:26:51 elingerf Exp $
C   $Log: net_setup.f,v $
C   Revision 1.15  2009/08/07 12:26:51  elingerf
C   no message
C
C   Revision 1.14  2009/08/07 12:26:14  elingerf
C   no message
C
C   Revision 1.13  2009/08/07 12:25:32  elingerf
C   no message
C
C   Revision 1.12  2009/08/07 12:22:55  elingerf
C   no message
C
C   Revision 1.11  2009/08/07 12:18:08  elingerf
C   no message
C
C   Revision 1.10  2009/08/07 12:12:56  elingerf
C   no message
C
C   Revision 1.9  2009/08/07 11:58:11  elingerf
C   no message
C
C   Revision 1.8  2009/08/07 11:44:32  elingerf
C   no message
C
C   Revision 1.7  2008/07/31 17:14:06  bucknerk
C   trivial
C
...
C
C   Revision 1.3  2007/10/04 15:21:26  bucknerk
C   This is from the latest version on Rafe's SVN tree.  It has been modified to
C   NOT print out the set of isotopes to stdout and NOT ask for a one-line
C   description. Other than that it is the same (I really hope). Kim
C
C   Revision 1.2  2007/07/23 16:28:42  bucknerk
C   This is a newer version of net_setup (I think) that has been tweaked to
C   produce the type of output the CINA suite is looking for. Other than that
C   there are no changes from the original.
C
C   Revision 1.1  2007/07/12 19:03:54  bucknerk
C   Added line to move compiled net_setup binary to the mc directory for testing
C   purposes.
C
C
C
C Look for Write(6 lines that have been commented out.
!===============================================================================
...
!-----------------------------------------------------------------------------
      use nuc_number
      integer, parameter :: nrm1=32000,nrm2=50000,nrm3=100
...
!-------------------------------------------------------------------
     ...
     &   'ld11','hd08','ua08','ol11','ls09','ia08','cb09','dc11',
     &   'ma10','mb07','ls09','dc11','mv09','bh06'/)
...
!      Write(6,"(1x,'n=',i4)") ny
!      Commented out previous 2 and ADDED next 2 for CINA
       Write(6,"('Library has ',I0,' isotopes')" ) ny
       Write(6,"('Isotopes vary from',A6,' to',A6)")(nname(1),nname(ny))
...
!  Output the nucleus to reaction registration to nets4
      ...
      Enddo
! These lines commented out because not needed for the online version
...
!      Write(6,"(3(10x,i8))") nm
! Added for the online version
       Write(6,"('Number of 7 parameter sets:')")
       Do i=1,8
         Write(6,"(A18,I2,A5,I0,A15)") 'reaction type',i,'has '
     &  ,(le(i)-la(i)+1),'parameter sets'
       Enddo
       Write(6,"('Number of rates not parameterized:',i6)") nm
       Write(6,"('Network prepared for simulation')")
...
!  Read in sunet
  ...
  100 ny=i-1
      Close(7)
! Commented out for on-line version
!      Write(6,*) ny
